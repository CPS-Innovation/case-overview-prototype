...
async function createColleagueCase(prisma, prosecutor, paralegalOfficer, config) {
  ...
  const forceHasHearing = (defendantStatus === statuses.CHARGED && needsReview) ? nextForcedHasHearing(prosecutor.id) : undefined
  await addHearings(prisma, { caseId: _case.id, unitId, defendants: [defendant, ...extraDefendants], status: defendantStatus, forceHasHearing })
...
async function createReviewCase(prisma, user, taskName, config, hasCharge = true) {
  const { defenceLawyers, charges, firstNames, lastNames, victims, types, complexities, policeUnits, ukCities, documentNames, documentTypes } = config;
...
async function seedBruceCases(prisma, dependencies, config) {
  ...
  await createReviewCase(prisma, bruceBanner, 'Make charging decision', fullConfig);
  await createReviewCase(prisma, bruceBanner, 'Make charging decision', fullConfig, false);

  return TEST_CASES.length + 20 + 1 + 1 + 2;
}
