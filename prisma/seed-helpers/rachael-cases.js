...
async function createManyWitnessesCase(prisma, user, config) {
  ...
  const forceHasHearing = (status === statuses.CHARGED && needsReview) ? nextForcedHasHearing(user.id) : undefined
  await addHearings(prisma, { caseId: _case.id, unitId: RACHAEL_UNITS.WESSEX_CROWN_COURT, defendants: [defendant, ...extraDefendants], status, forceHasHearing })
...
async function seedRachaelCases(prisma, dependencies, config) {
  ...
  await createReviewCase(prisma, rachaelHarvey, 'Make charging decision', fullConfig);
  await createReviewCase(prisma, rachaelHarvey, 'Make charging decision', fullConfig, false);

  return RACHAEL_TASKS.length + 1 + 20 + 1 + 1 + 2;
}
